...
if(isset($_GET['bill_value'])){
    $bill_value =  $_GET['bill_value'] ;
// $_SESSION['vvv'] =$_GET['bill_value'] ;
}
    ...
    <div class="row">
        <form action="student_bills.php" method="GET" class="col-6  "  >
            <div class="form-group ">
                ...
                <div class="col-lg-6" ><input type="number" id="new_value" name="new_value"  class="form-control  " placeholder="المصاريف" ></div>
                <div class="col-lg-3" ><button type="submit"  class="btn btn-outline-primary " >إضافه</button></div>
            </div>
        </form>
        <form action="student_bills.php" method="GET" class="col-6 " >
            <div class="form-group  ">
                <div class="col-lg-6" ><label for="bill_value" > مبلغ الدفع</label></div>
                <div class="col-lg-6" ><input type="number" id="bill_value" name="bill_value"  class="form-control"  placeholder="<?php if(isset($bill_value)){ echo $bill_value; }?>" ></div>
                <div class="col-lg-3" ><button type="submit"  class="btn btn-outline-primary " >إضافه</button></div>
            </div>
        </form>
            </div>
...
// i was hiding ahahhaha

// if(isset($_GET['new_value'])){
//     $new_value =  $_GET['new_value'] ;
//     $sql_add_total = "UPDATE students SET total = (total+$new_value) WHERE id = $student_id ";
//     $query_add_total = mysqli_query($connection , $sql_add_total) ;
//     header("Location:student_bills.php") ;
//     }
...
while($student = mysqli_fetch_assoc($student_data)){  ?>
        <tr>
            ...
            <td><?php if(isset($student['paid']) ){ echo $student['paid'] ;}?></td>
            <td><?php if(isset($student['total']) ){ echo $student['total'] ; }?></td>
            <td class="noprint"><a href="student_operations.php?id=<?php echo $student['id']; ?> & action=bills & value=<?php if(isset($bill_value)){ echo $bill_value; }?>" class="btn btn-success">دفع المبلغ</a>
